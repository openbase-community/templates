...
When defining Django models, never make a stored property where a computed property will suffice. If there are invariants that should be maintained regarding the field values, add checks in `clean` and trigger it from `save`, or else simply fill in default values in the model's `save` method, remembering to call the superclass `save`.
